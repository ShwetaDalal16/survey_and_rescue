...
class sr_determine_colors():
	...
	def load_rois(self):
		try:
			...
			folder = os.environ['ROS_PACKAGE_PATH'].split(':')[0]
			package = 'survey_and_rescue'
			file_name = 'rect_info.pkl'
			file_path = os.path.join(folder, package, 'scripts', file_name)
...
def main(args):
	...
	while not rospy.is_shutdown():
		try:
			s.detect_color_contour_centers('red')
			s.check_whether_lit()
			rate.sleep()
		except KeyboardInterrupt:
			cv2.destroyAllWindows()
